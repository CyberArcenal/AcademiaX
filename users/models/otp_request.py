...
class OtpRequest(models.Model):
    ...
    type = models.CharField(
        max_length=10, choices=OtpRequestType.choices, default=OtpRequestType.EMAIL
    )
    is_email_delivered = models.BooleanField(default=False)
    is_phone_delivered = models.BooleanField(default=False)

    def clean(self):
        if self.type not in dict(OtpRequestType.choices):
    ...
    def __str__(self):
        return f"OTP for {self.user.username if self.user else 'unknown'} - {self.otp_code}"