...
from hr.state_transition_services.leave import LeaveRequestStateTransitionService
...
def leave_post_save(sender, instance, created, **kwargs):
    ...
    if created:
        LeaveRequestStateTransitionService.handle_creation(instance)
        return

    if hasattr(instance, '_state_transition_changes'):
        changes = instance._state_transition_changes
        LeaveRequestStateTransitionService.handle_changes(instance, changes)
